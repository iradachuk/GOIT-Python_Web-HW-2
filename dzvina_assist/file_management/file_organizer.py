...
def sorting_files(path) -> str:
    ...
    paths_dict = sort_by_extension(paths_list)
    ...
    for file in paths_list:
        if Path(file).parent == work_path:
            ...
            try:
                ...
                copy_count = list(map(str, Path(work_path).glob(f'{steam}*{suffix }'))) # str(len(direct_processing(home_path, Path(file).stem, Path(file).suffix)))
...
def sort_by_extension(paths_list: list) -> dict:
    ...
    audio_extensions = ('.mp3', '.ogg', '.wav', '.amr', '.m4a')
    archives_extensions = ('.zip', '.gz', '.tar', '.7z', '.rar')
    ...
    for path in paths_list:
        ...
        if suffix in images_extensions:
        ...
        elif suffix in archives_extensions:
